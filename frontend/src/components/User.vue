				...
				<div class="flex justify-between items-center py-6 md:justify-start md:space-x-10">
					...
					<div class="md:flex items-center justify-end md:flex-1 lg:w-0">
						<button @click="toggleUserPanel"
							...
							{{ currentUser ? currentUser.username : 'Manage User' }}
						</button>
					</div>
				</div>
			</div>
			...
			<div
				...
				<div class="px-5 pt-5 pb-6 space-y-6">
					<div v-if="currentUser">
						<h2 class="text-lg font-medium text-gray-900">Current User: {{
							currentUser.username }}</h2>
						<button @click="updateUser"
							class="mt-3 w-full inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700">
							Update User
						</button>
...
export default {
	...
	methods: {
		toggleUserPanel() {
			...
			console.log('showUserPanel:', this.showUserPanel);
		},
		async createUser() {
			try {
